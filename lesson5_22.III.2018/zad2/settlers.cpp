...
Household::Household(unsigned int inh_m, unsigned int init_size)
{
    inh_max = inh_m;
    inh_current = init_size;
    inhabitants = new Villager*[inh_current];
    for(unsigned int i = 0; i < inh_current; i++)
        inhabitants[i] = new Villager();
}

void Household::next_year()
{
    for(unsigned int i = 0; i < inh_current; i++)
        inhabitants[i]->aging();
}
